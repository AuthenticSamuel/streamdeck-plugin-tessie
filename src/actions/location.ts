...
@action({ UUID: "dev.infernal.tessie.location" })
export class LocationAction extends SingletonAction {
  #tessie?: Tessie;

  override async onWillAppear(
    event: WillAppearEvent<InstanceSettings>,
  ): Promise<void> {
    this.#updateKey(event);
  }

  override async onKeyDown(
    event: KeyDownEvent<InstanceSettings>,
  ): Promise<void> {
    this.#updateKey(event);
  }
  ...
  async #updateKey(ev: UpdateKeyEvent) {
    ...
    const data = await this.#tessie.getLocation();
    ...
    const map = await this.#tessie.getMap();
    if (map) {
      ev.action.setImage(map);
    } else {
      ev.action.setTitle(data.address);
    }
  }
}
