...
@action({ UUID: "dev.infernal.tessie.battery" })
export class BatteryAction extends SingletonAction {
  ...
  override async onDidReceiveSettings(
    ev: DidReceiveSettingsEvent<InstanceSettings>,
  ): Promise<void> {
    this.#updateKey(ev);
  }
  ...
  async #updateKey(ev: UpdateKeyEvent) {
    ...
    const data = await this.#tessie.getBattery();
    ...
    ev.action.setTitle(`${Math.round(data.battery_level).toString()}%`);

    const svg = createRingSVG(data.battery_level);
